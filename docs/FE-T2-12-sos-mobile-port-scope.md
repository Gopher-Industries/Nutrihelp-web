...
- `src/components/EmergencySOS/EmergencySOSButton.jsx` — persistent FAB
...
**One-tap calling**
9. With a valid contact saved, the FAB is red and labelled "SOS".
10. A single tap opens the **system dialer pre-filled** with the saved number — verified on a **physical
    Android device and a physical iOS device** (the iOS Simulator cannot do this).
11. Number normalisation matches web: spaces, parentheses and hyphens stripped, a leading `+` preserved.
12. If the contact is somehow invalid or unnormalisable, the editor opens instead of a dial attempt.
13. On a device without telephony, the rejected `openURL` produces a clear
    "This device can't make phone calls" alert — never a silent failure or crash.
14. **No `CALL_PHONE` permission is declared or requested**, and no runtime permission prompt appears
    anywhere in the flow.

**Placement and visibility**
15. The FAB is visible on every authenticated screen across all six tabs and inside nested stacks.
16. The FAB is **not** rendered on the auth stack (login/signup).
17. The FAB does not overlap the FloatingChatbot FAB; it sits above it with a consistent gap.
18. Touches that miss the FAB reach the content underneath (`pointerEvents="box-none"`).
19. The FAB respects safe-area insets and never sits under the tab bar or home indicator.
20. The Profile → Settings → "Emergency Contact" row opens the same editor and stays in sync with the
    FAB state without needing an app restart.

**Accessibility**
21. Touch target is at least 64×64.
22. `accessibilityRole="button"` is set, and `accessibilityLabel` reflects state — "Call emergency
    contact &lt;name&gt;" when ready, "Add emergency contact" when in setup.
23. The button and its label scale correctly at all three `FONT_SIZE_OPTIONS` settings (normal, large,
    extra large) with no clipping or overlap.
24. Contrast of the red fill against white and dark themes meets WCAG AA for large text.

**Quality**
25. Unit tests for the utility module cover validation boundaries (7, 8, 15 and 16 digits), `tel:`
    normalisation including the `+` case, user-key normalisation and the guest fallback.
26. The editor copy states that the contact is stored on this device only — setting the expectation
    until backend sync ships.
27. No regression in the FloatingChatbot's position or tap behaviour.

#### Test notes

- `tel:` cannot be exercised in the iOS Simulator; criteria 10 requires physical devices.
- Verify criterion 13 on a Wi-Fi-only tablet or an emulator without telephony.
- Confirm no `AndroidManifest.xml` `<queries>` change is needed, which holds as long as the
  implementation avoids `Linking.canOpenURL` (Section 3.2).
